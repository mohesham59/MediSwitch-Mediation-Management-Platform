...
public class CDRParser {
     ...
     *
     * Expected file format (key=value per line): record_type=VOICE
     * calling_number=01012345678 called_number=01098765432
     * start_time=2026-05-08T14:30:00 duration=120 bytes_sent=0 bytes_received=0
     * status=SUCCESS
     */
    ...
    // ── Helpers ──────────────────────────────────────────────────────────────
    /**
     * Derives the node source from the filename prefix. msc_cdr_* → MSC
     * pgw_cdr_* → PGW smsc_cdr_* → SMSC
     */
    private String extractSourceFromFilename(String filename) {
        String lower = filename.toLowerCase();
        if (lower.startsWith("msc_")) {
            return "MSC";
        }
        if (lower.startsWith("pgw_")) {
            return "PGW";
        }
        if (lower.startsWith("smsc_")) {
            return "SMSC";
        }
        return "UNKNOWN";
    }
     ...
     * msc_cdr_20260508_143016.txt → 20260508_143016 Falls back to the current
     * epoch string if the pattern doesn't match.
     */
    ...
    /**
     * Warns about missing expected fields based on the source type. Does NOT
     * throw — validation is advisory only; caller decides what to do.
     */
    private void validateRecord(CDRRecord record, String filename) {
        ...
        switch (source) {
            case "MSC":
                required = new String[]{"caller_id", "receiver_id", "start_time",
                    "duration", "hplmn", "vplmn",
                    "external_charges", "rated_flag"};
                break;
            case "PGW":
                required = new String[]{"imsi", "session_start", "session_duration",
                    "data_usage_mb", "apn", "hplmn", "vplmn",
                    "external_charges", "rated_flag"};
                break;
            case "SMSC":
                required = new String[]{"sender_id", "receiver_id", "timestamp",
                    "message_length", "service_type", "hplmn",
                    "vplmn", "external_charges", "rated_flag"};
                break;
            default:
                return;
        }
